...
def main():
    ...
    logger.info('================== Running with seed {} =========================='.format(args.seed))
    ...
    if args.fp16:
        ...
        logger.info("warm up linear: warmup = {}, t_total = {}.".format(warmup_linear.warmup,warmup_linear.t_total))
    else:
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            logger.info('Running at Epoch {}'.format(epoch))
            # Train
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", dynamic_ncols=True)):
                ...
                if (step + 1) % args.per_eval_step == 0 or step == len(train_dataloader) - 1:
                    ...
                    if args.save_all:
                        ...
                        output_model_file = os.path.join(args.output_dir, "pytorch_model_{}.bin".format(eval_epoch))
                    ...
                    eval_epoch += 1
            logger.info('Epoch {}: Accuracy: {}, Train Loss: {}'.format(epoch,best_acc,train_loss.avg))
    ...
    for output_model_name in ["pytorch_model.bin", "pytorch_loss_model.bin"]:
        ...
        if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            ...
            logger.info("Accuracy on Test set: {}".format(test_acc.get_metric(reset=True)))
    ...
    if args.metric == 'accuracy':
        ...
        raise RuntimeError("Wrong metric type for {}, which must be in ['accuracy', 'loss'].".format(args.metric))
    model_state_dict = torch.load(output_model_file, map_location='cuda:0')
    model = initialize_model(args.bert_name, args.model_file, state_dict=model_state_dict, **model_params)
    model.to(device)
