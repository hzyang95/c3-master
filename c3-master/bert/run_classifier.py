...
class c3Processor(DataProcessor):
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(tqdm(data)):
            ...
            label = self.convert_to_unicode(answer)

            for k in range(4):
                guid = "%s-%s-%s" % (set_type, i, k)
                text_a = self.convert_to_unicode(data[i][0])
                text_b = self.convert_to_unicode(data[i][k + 2])
                text_c = self.convert_to_unicode(data[i][1])
                examples.append(
